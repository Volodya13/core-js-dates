...
function getDayName(date) {
  const dateObj = new Date(date);
  const weekDay = dateObj.toLocaleString('en-EN', {
    weekday: 'long',
    timeZone: 'UTC',
  });
  return weekDay;
...
function getNextFriday(date) {
  ...
  const untilNExtFriday = (12 - currentDate.getUTCDay()) % 7 || 7;
  currentDate.setUTCDate(currentDate.getUTCDate() + untilNExtFriday);
  return currentDate;
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.abs(dateEndObj - dateStartObj) / (1000 * 60 * 60 * 24) + 1;
}
...
function formatDate(date) {
  ...
  const partOfTheDay = hours >= 12 ? 'PM' : 'AM';
  hours %= 12;
  hours = hours || 12;
  hours = hours.toString();

  return `${month}/${day}/${year}, ${hours}:${minutes}:${seconds} ${partOfTheDay}`;
}
